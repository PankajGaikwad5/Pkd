...
export default function ProjectsClient() {
  ...
  // Sync state values inside ref for scroll handler
  const stateRef = useRef({ activeIndex, isTransitioning });
  useEffect(() => {
    stateRef.current = { activeIndex, isTransitioning };
  ...
  useEffect(() => {
    ...
    const handleWheelEvent = (e) => {
      ...
      const { isTransitioning: lock } = stateRef.current;
      if (lock) return;
  ...
  return (
    <>
      ...
      <main
        ref={containerRef}
        className="h-screen w-screen overflow-hidden relative bg-[#332820] text-[#D6CBBC] select-none flex flex-col justify-between"
      >
